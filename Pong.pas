...
procedure auto_play(start_vel:integer);
begin
...
while True do
  begin
    ...
    if (b.x in [rp.x-b.rad..rp.x+rp.size_x+b.rad]) and (b.y in [rp.y..rp.y+rp.size_y]) then
      begin
        b.vel_x:=-b.vel_x;
        b.vel_y+=random(rand_y)-trunc(rand_y/2);
      end;
    // отскок от левой ракетки
    if (b.x in [lp.x-b.rad..lp.x+lp.size_x+b.rad]) and (b.y in [lp.y..lp.y+lp.size_y]) then
      begin
        b.vel_x:=-b.vel_x;
        b.vel_y+=random(rand_y)-trunc(rand_y/2);
      end;
    ...
    auto_play(4);// автоматическая игра левой ракетки
