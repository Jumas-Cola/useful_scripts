...
class Utils:
    ...
    def upd_tmblr_start(self, file='state.json'):
        ...
        if self.posting_type == 1:
            for i in range(1000):
                if self.tumblr_user not in state:
                    state[self.tumblr_user] = self.get_tmblr_count() - 1
                else:
            ...
            for i in range(1000):
                state[self.tumblr_user] = random.randint(0, self.get_tmblr_count() - 1)
                post = self.get_tmblr_post(state[self.tumblr_user])
        ...
        with open(file, 'w') as f:
            f.write(json.dumps(state))
        return state[self.tumblr_user]

    def make_vk_post(self, tmblr_post, group_id, message=None):
        from_url = tmblr_post['url']
        if message is None:
            photo_caption = tmblr_post['photo-caption'] if 'photo-caption' in post else None
            message = """{caption}
